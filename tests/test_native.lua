...
describe("Native: Lists", function()
  ...
  it("renders nested unordered lists", function()
    local html = convert("- Parent\n  - Child1\n  - Child2")
    assert_contains(html, "Parent")
    assert_contains(html, "<ul")
    assert_contains(html, "Child1")
    assert_contains(html, "Child2")
    local _, count = html:gsub("<ul", "")
    assert_eq(count, 2, "expected 2 <ul> tags for nested unordered list")
  end)

  it("renders nested ordered lists", function()
    local html = convert("1. Parent\n   1. Child1\n   2. Child2")
    assert_contains(html, "Parent")
    assert_contains(html, "Child1")
    assert_contains(html, "Child2")
    local _, count = html:gsub("<ol", "")
    assert_eq(count, 2, "expected 2 <ol> tags for nested ordered list")
  end)

  it("renders mixed nested lists (ordered inside unordered)", function()
    local html = convert("- Bullet\n  1. Numbered")
    assert_contains(html, "<ul")
    assert_contains(html, "<ol")
    assert_contains(html, "Bullet")
    assert_contains(html, "Numbered")
  end)

  it("renders mixed nested lists (unordered inside ordered)", function()
    local html = convert("1. Numbered\n   - Bullet")
    assert_contains(html, "<ol")
    assert_contains(html, "<ul")
    assert_contains(html, "Numbered")
    assert_contains(html, "Bullet")
  ...
  it("renders three levels of nesting", function()
    ...
    assert_contains(html, "L2")
    assert_contains(html, "L3")
    local _, count = html:gsub("<ul", "")
    assert_eq(count, 3, "expected 3 <ul> tags for three-level nesting")
  end)

  it("keeps siblings flat when not indented", function()
    local html = convert("- A\n- B\n- C")
    local _, count = html:gsub("<ul", "")
    assert_eq(count, 1, "expected 1 <ul> tag for flat list")
  end)
end)
